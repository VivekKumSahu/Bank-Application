       ...
       try
       {
    	   ...
    	   if(rs.next())
    	   {
    		   rs.getString(17);
           	   ...
           	   {
           		    PreparedStatement ps1=con.prepareStatement("select * from AmountTransaction where AccountNo=? order by TransactionDate desc");
              	    ...
              	    while(rs1.next())
              	    {
            		     ...
            		     String TransDate=td.toString().substring(8,10)+"-"+td.toString().substring(5,7)+"-"+td.toString().substring(0,4);
